...
class InstanSeg:
    ...
    def read_image(self, image_str: str, processing_method = "auto") -> Union[Tuple[str, float], Tuple[np.ndarray, float]]:
        ...
        :param processing_method: The processing method to use. Options are "auto", "small",
            "medium", "wsi". If "auto", the method will be chosen based on the size of the image.
        :return: The image array if it can be safely read (or the path to the image if it cannot)
            and the pixel size in microns.
        """
        if self.prefered_image_reader == "tiffslide":
            ...
            raise NotImplementedError(
                f'Image reader {self.prefered_image_reader} is not implemented.'
            )
    ...
    def _get_eval_function_to_use(self, num_pixels, processing_method='auto') -> str:
        if processing_method != 'auto':
            assert processing_method in ['small', 'medium', 'wsi'], (
                f'Processing method {processing_method} is not supported.'
            )
    ...
    def read_slide(self, image_str: str):
        ...
        """
        raise NotImplementedError(
            f'Image reader {self.prefered_image_reader} is not implemented for whole slide images.'
        )
        return slide
    ...
    def eval(
        ...
        **kwargs,
        ...
        for image in image_list:
            image_array, img_pixel_size = self.read_image(
                image, processing_method=processing_method
            )
            ...
            if img_pixel_size is None:
                ...
                warnings.warn(
                    'Pixel size not provided and could not be read from image metadata, this may lead to innacurate results.'
                )
            ...
            if not isinstance(image_array, str):
                ...
                if eval_function_str == 'small':
                    instances = self.eval_small_image(
                        image=image_array,
                    ...
                    instances = self.eval_medium_image(
                        image=image_array,
                        pixel_size=img_pixel_size,
                        return_image_tensor=False,
                        **kwargs,
                    ...
                    raise NotImplementedError(
                        f'Processing method {eval_function_str} is not implemented for image array inputs.'
                    )
                ...
                self.eval_whole_slide_image(
                    image_array, pixel_size, save_geojson=save_geojson, **kwargs
                )
    ...
    def save_output(
        ...
        if save_geojson:
            ...
            if output_dimension == 1:
                ...
                features = (
                    labels_to_features(
                        labels[0, 0], object_type='detection', classification='Nuclei'
                    )['features']
                    + labels_to_features(
                        labels[0, 1], object_type='detection', classification='Cells'
                    )['features']
                )
        ...
        **kwargs,
        ...
        if 'channel_ids' in kwargs:
            assert max(kwargs['channel_ids']) <= image.shape[1], (
                f'Number of channel ids {(kwargs["channel_ids"])} does not match number of channels in image {image.shape[1]}.'
            )
        ...
        if normalise:
            image = _to_ndim(image, 4)
            image = torch.stack(
                [percentile_normalize(i) for i in image]
        ...
        **kwargs,
        ...
        if img_pixel_size is None or img_pixel_size > 1 or img_pixel_size < 0.1:
            ...
            if pixel_size is not None:
                ...
                raise ValueError(
                    'The image pixel size {} is not in microns.'.format(img_pixel_size)
                )
        ...
        dims = (
            round(dims[1] / scale_factor),
            round(dims[0] / scale_factor),
        )  # The dimensions are opposite to numpy/torch/zarr dimensions.
        ...
        counter = -1
        for _, ((i, window_i), (j, window_j)) in tqdm(
            enumerate(product(enumerate(chop_list[0]), enumerate(chop_list[1]))),
            total=total,
            colour='green',
            desc='Slide progress: ',
        ):
            ...
            if (
                new_tile.shape[-2:] != shape
            ):  # this only happens when the pixel size is close but not exactly the model pixel size.
            ...
            for n in range(num_iter):
                ...
                tile1 = canvas[
                    n, window_i_start : window_i + shape[0], window_j_start : window_j + shape[1]
                ]
                tile2 = _remove_edge_labels(
                    new_tile[
                        n,
                        window_i_start - window_i : shape[0],
                        window_j_start - window_j : shape[1],
                    ],
                    ignore=ignore_list,
                )
                if edge_window:
                    ...
                    canvas[
                        ...
                        window_i_start : window_i + shape[0],
                        window_j_start : window_j + shape[1],
                    ] = tile1_torch.numpy().astype(np.int32)

                else:
                    tile1 = canvas[
                        n,
                        window_i + pad : window_i + shape[0] - pad,
                        window_j + pad : window_j + shape[1] - pad,
                    ]
                    tile2 = _remove_edge_labels(
                        new_tile[n, pad : shape[0] - pad, pad : shape[1] - pad]
                    )
    ...
    def display(
        self, image: torch.tensor, instances: torch.Tensor, normalise: bool = True
    ) -> np.ndarray:
        """
    ...
    def _cluster_instances_by_mean_channel_intensity(self, image_tensor: torch.Tensor, 
        ...
        from instanseg.utils.utils import _choose_device, apply_cmap
        ...
        if features is None:
            X_features = get_mean_object_features(
                image_tensor.to(device),
                labeled_output.to(device),
            )
        else:
            X_features = features
        ...
        mapping = fastremap.component_map(
            np.arange(1, len(adata.obs['leiden']) + 1), adata.obs['leiden'].astype(np.int64) + 1
        )
...
def _threshold_thumbnail(slide, level=None, sigma=3):
    ...
    img_thumbnail = slide.read_region(
        (0, 0), level, size=(10000, 10000), as_array=True, padding=False
    )
...
def _find_non_empty_positions(
    mask, chop_list, tile_size, chopped_image_size, emptiness_threshold=0.1
):
    """
...
def _rescale_to_pixel_size(
    ...
    if not np.allclose(
        scale_factor, 1, pixel_size_precision
    ):  # if you change this value, you MUST modify the whole_slide_image function.
...
def _display_colourized(mIF, normalise=True):
    ...
    if mIF.shape[0] != 3:
        ...
        colour_render = (mIF.flatten(1).T @ torch.tensor(colours)).reshape(
            mIF.shape[1], mIF.shape[2], 3
        )
    else:
        colour_render = mIF
